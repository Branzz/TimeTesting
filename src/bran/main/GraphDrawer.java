...
public class GraphDrawer extends Applet {
...
//    private Method method;
    ...
    public GraphDrawer(String[] methodStrings, int maxSize, int trialAmount, int testCases) {
...
//    	return (int) (Math.log(caseSize) * testCases / Math.log(maxSize));
	...
	public synchronized void init() {
		...
		for (String methodString : methods) { // #$stream
			IdentityHashMap<Object, Trio<Method, ArrayList<Long>, Color>> collectionMapsData = new IdentityHashMap<Object, GraphDrawer.Trio<Method,ArrayList<Long>,Color>>(
...
//						System.out.println(collectionMapsData.get(colMap).a);
			...
			for (Entry<Object, Trio<Method, ArrayList<Long>, Color>> entry : collectionMapsData.entrySet()) {
				entry.getValue().c = new Color(Color.HSBtoRGB(hueShift++ / collectionMapsData.size(), .5f - .1f * satShift / methods.length, .6F));
			}
